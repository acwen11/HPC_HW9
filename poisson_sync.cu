...
// This version of the code uses block level synchronization
// Hence only 1 block can be used. If multiple blocks are used
// the iterations are not strictly speaking Jacobi, and the
// convergence is slow.
...
int main(int argc, char **argv)
{
 ...
 // const gpu_fp t_final = atof(argv[1]);
  ...
  int blockSize = 256;
  int numBlocks = 1;  // Need there to be only 1 block for this to work
  ...
  for (; ; )
  {
    kernel <<<numBlocks, blockSize>>> (gfs_managed);
